...
export default function WiredNtkCard({handleNext}) {
  const [visibleCards, setVisibleCards] = useState(12);
  const [searchQuery, setSearchQuery] = useState("");
  
  const WiredCardWithIds = WiredCard.map((card, index) => ({
      ...card,
      id: index + 1
    }));
  ...
  const filteredCards = WiredCardWithIds.filter(card =>
    ...
    <div>
      ...
      <div className={styles.Grid}> 
        {filteredCards.slice(0, visibleCards).map((wiredcard) => (
          <CardItem key={wiredcard.id} data={wiredcard} handleNext={handleNext}/>
        ))}
      </div>
